...
function BugSettingsMenu({ bugVisualSettings, containerRef, onChange, onMenuToggle, open, showParticleCount, terminatorMode, onToggle }) {
  return (
    <div className="relative" ref={containerRef}>
      <Tooltip content="Bug field controls for size, speed, and debug overlays.">
        <button
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+10px)] z-30 grid w-[320px] gap-3 rounded-[20px] border border-white/10 bg-zinc-950/96 p-4 text-sm text-stone-200 shadow-[0_24px_48px_rgba(0,0,0,0.34)] backdrop-blur-xl">
          ...
          />
          <Toggle checked={terminatorMode} description="Swap the normal bug field for a bug-smashing mini-game with a hammer cursor." label="Terminator mode" onChange={() => onToggle('terminatorMode')} />
