function Toggle({ checked, description, label, onChange }) {
  return (
    <label className="flex items-start justify-between gap-4 rounded-[16px] border border-white/6 bg-white/[0.03] px-3 py-3">
...
function SettingsMenu({ settings, onToggle }) {
  return (
    <details className="relative">
      ...
      <div className="absolute right-0 top-[calc(100%+10px)] z-30 grid w-[320px] gap-3 rounded-[20px] border border-white/10 bg-zinc-950/96 p-4 text-sm text-stone-200 shadow-[0_24px_48px_rgba(0,0,0,0.34)] backdrop-blur-xl">
        <span className="text-sm font-medium text-stone-300">Settings</span>
        ...
        <Toggle checked={settings.showParticleCount} description="Show the live background particle count for QA while tuning the bug field." label="Show bug particle count" onChange={() => onToggle('showParticleCount')} />
      </div>
    </details>
  )
}
