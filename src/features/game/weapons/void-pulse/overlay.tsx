...
export function VoidPulseOverlay({
  ...
  const coreDiameter = Math.max(
    44,
    shockwaveRadius * (0.78 + chaosScale * 0.06),
  );
  const pulseDiameter = Math.max(
    88,
    shockwaveRadius * (1.78 + chaosScale * 0.18),
  );
        ...
        >
          ...
          >
            {Array.from({ length: markerCount }).map((_, index) => {
              ...
              const tangentDy =
                Math.sin(angle + Math.PI / 2) * markerLength * 0.72;
          ...
          {tier >= 2 ? (
            ...
            >
              {Array.from({ length: tier === 3 ? 4 : 3 }).map((_, index) => {
                ...
                const controlX =
                  svgRadius +
                  Math.cos((start + end) / 2) * outerOrbitRadiusX * 1.04;
                const controlY =
                  svgRadius +
                  Math.sin((start + end) / 2) * outerOrbitRadiusY * 1.04;
