...
test.describe("dashboard settings QA", () => {
  test("updates deadline math and persists consolidated settings", async ({ page }) => {
    ...
    await page.getByText("Show background bugs").click();
  ...
  test("hides the game launcher when background bugs start disabled", async ({ page }) => {
    const clientErrors = createConsoleCollectors(page);

    await page.setViewportSize({ height: 1200, width: 1440 });
    await seedDashboardState(page, {
      clearStorage: false,
      showAmbientBugs: false,
    });
    await page.goto("./");
